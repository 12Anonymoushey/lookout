...
export default function AdminView({ vehicles = [], onCloseDrawer, drawerOpen, collapsed, onToggleCollapse }) {
  ...
  const panel = (
    <>
      ...
      <Section icon={Users} title="Overview" tint="cyan">
        <div className="grid grid-cols-3 gap-2">
          {[{ label: 'Commuters', value: counts.commuter, cls: 'text-[var(--lo-cyan)]' }, { label: 'Drivers', value: counts.driver, cls: 'text-[var(--lo-sky)]' }, { label: 'Admins', value: counts.admin, cls: 'text-[#c39bd3]' }].map((stat) => (
            <div key={stat.label} className="rounded-xl border border-[var(--lo-card-border)] bg-[var(--lo-card)]/50 p-2 text-center">
              <p className={`font-mono text-base font-bold ${stat.cls}`}>{stat.value}</p>
              <p className="text-[9px] uppercase tracking-wide text-[var(--lo-text-secondary)]">{stat.label}</p>
            </div>
          ))}
        </div>
      ...
      <Section icon={Radio} title="Live Now" defaultOpen={false} tint="cyan">
        {vehicles.length === 0 ? (
          <p className="text-[11px] text-[var(--lo-text-secondary)]">No vehicles broadcasting.</p>
        ) : (
          <ul className="space-y-1.5">
            {vehicles.map((vehicle) => {
              const meta = statusMeta(vehicle);
              return (
                <li key={vehicle.vehicleId} className="flex items-center justify-between gap-2 rounded-xl border border-[var(--lo-card-border)] bg-[var(--lo-card)]/40 px-2.5 py-1.5">
                  <span className="truncate font-mono text-[11px] font-bold text-[var(--lo-text)]">{vehicle.vehicleId}</span>
      ...
      <Section icon={Hand} title="Recent Pokes" defaultOpen={false} tint="pink">
        {pokes.length === 0 ? (
          <p className="text-[11px] text-[var(--lo-text-secondary)]">No pokes yet.</p>
        ) : (
          <ul className="max-h-64 space-y-1.5 overflow-y-auto">
            {pokes.slice(0, 25).map((poke) => (
              <li key={poke.id} className="rounded-xl border border-[var(--lo-card-border)] bg-[var(--lo-card)]/40 px-2.5 py-2">
                <p className="flex items-center justify-between gap-2 text-[11px] font-bold text-[var(--lo-text)]">
                  <span className="truncate font-mono">{poke.toPlate}</span>
                  <span className="shrink-0 text-[10px] font-normal text-[var(--lo-text-secondary)]">{timeAgo(poke.createdAt)}</span>
                </p>
                <p className="truncate text-[10px] text-[var(--lo-text-secondary)]">{poke.fromName}</p>
              </li>
            ))}
          </ul>
        )}
  ...
  return (
    <AppShell brand="Admin" panel={panel} open={drawerOpen} onCloseDrawer={onCloseDrawer} collapsed={collapsed} onToggleCollapse={onToggleCollapse}>
      <div className="h-full overflow-y-auto bg-[var(--lo-bg)] p-3 sm:p-5">
        <header className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <span className="rounded-full border border-[var(--lo-card-border)] bg-[var(--lo-card)]/60 px-3 py-1.5 text-[11px] font-bold text-[var(--lo-text)]">
          ...
          <div className="flex items-center gap-1.5">
            {['all', 'commuter', 'driver', 'admin'].map((r) => (
              <button
                ...
                className={`rounded-full px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-wide transition ${roleFilter === r ? 'bg-[var(--lo-pink)] text-white' : 'border border-[var(--lo-card-border)] bg-[var(--lo-card)]/40 text-[var(--lo-text)] hover:border-[var(--lo-pink)]/40'}`}
              >
                {r}
              </button>
            ))}
          ...
          {filtered.map((user) => (
            <div key={user.uid} className="flex items-center justify-between gap-3 rounded-2xl border border-[var(--lo-card-border)] bg-[var(--lo-card)]/30 px-4 py-3">
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-bold text-[var(--lo-text)]">{user.fullName || 'User'}</p>
                <p className="truncate text-[11px] text-[var(--lo-text-secondary)]">{user.email}</p>
                {user.phone && <p className="text-[10px] text-[var(--lo-text-secondary)]">{user.phone}</p>}
                <div className="mt-1 flex flex-wrap items-center gap-1.5">
                  ...
                  {user.plates?.map((p) => (
                    <span key={p} className="inline-flex items-center gap-1 rounded-full bg-[var(--lo-card)]/60 px-2 py-0.5 text-[10px] font-bold text-[var(--lo-text)]">
                      <CreditCard size={9} /> {p}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex items-center gap-1">
                {user.uid !== profile?.uid && (
                  <>
                    <button type="button" onClick={() => openEditor(user)} className="rounded-lg p-1.5 text-[var(--lo-text-secondary)] transition hover:bg-[var(--lo-card-border)] hover:text-[var(--lo-text)]" title="Edit">
                      <Pencil size={13} />
                    </button>
                      ...
                      {confirmUid === user.uid && (
                        <div className="absolute right-0 top-full z-50 mt-1 rounded-xl border border-rose-500/40 bg-[var(--lo-panel)] p-3 shadow-xl">
                          ...
                          <div className="flex gap-2">
                            ...
                            <button type="button" onClick={() => setConfirmUid('')} className="rounded-lg border border-[var(--lo-card-border)] px-2.5 py-1 text-[10px] font-bold text-[var(--lo-text)]">Cancel</button>
                          </div>
                        </div>
                      )}
                    </div>
      ...
      {/* Edit modal */}
      {editing && (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-[var(--lo-bg)]/80 backdrop-blur-sm">
          <div className="mx-4 w-full max-w-lg overflow-hidden rounded-2xl border border-[var(--lo-card-border)] bg-[var(--lo-panel)] shadow-2xl">
            <header className="flex items-center justify-between border-b border-[var(--lo-panel-border)] px-5 py-3">
              <h3 className="text-sm font-bold text-[var(--lo-text)]">Edit {editing.email}</h3>
              <button type="button" onClick={() => setEditing(null)} className="rounded-lg p-1 text-[var(--lo-text-secondary)] hover:text-[var(--lo-text)]"><X size={16} /></button>
            </header>
            <div className="space-y-3 p-5">
              <label className="block">
                <span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-[var(--lo-text-secondary)]">Full name</span>
                <input value={draft.fullName} onChange={(e) => setDraft((d) => ({ ...d, fullName: e.target.value }))} className="w-full rounded-xl border border-[var(--lo-card-border)] bg-[var(--lo-card)] px-3 py-2 text-sm text-[var(--lo-text)] focus:border-[var(--lo-pink)] focus:outline-none" />
              </label>
              <label className="block">
                <span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-[var(--lo-text-secondary)]">Mobile</span>
                <input value={draft.phone} onChange={(e) => setDraft((d) => ({ ...d, phone: e.target.value }))} className="w-full rounded-xl border border-[var(--lo-card-border)] bg-[var(--lo-card)] px-3 py-2 text-sm text-[var(--lo-text)] focus:border-[var(--lo-pink)] focus:outline-none" />
              </label>
              <label className="block">
                <span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-[var(--lo-text-secondary)]">Role</span>
                <select value={draft.role} onChange={(e) => setDraft((d) => ({ ...d, role: e.target.value }))} className="w-full appearance-none rounded-xl border border-[var(--lo-card-border)] bg-[var(--lo-card)] px-3 py-2 text-sm text-[var(--lo-text)] focus:border-[var(--lo-pink)] focus:outline-none">
              ...
              {draft.role === 'driver' && (
                <label className="block">
                  <span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-[var(--lo-text-secondary)]">Plate numbers (comma-separated)</span>
                  <input value={draft.plates} onChange={(e) => setDraft((d) => ({ ...d, plates: e.target.value }))} className="w-full rounded-xl border border-[var(--lo-card-border)] bg-[var(--lo-card)] px-3 py-2 text-sm text-[var(--lo-text)] focus:border-[var(--lo-pink)] focus:outline-none" />
                </label>
              )}
            </div>
            <footer className="flex items-center justify-end gap-2 border-t border-[var(--lo-panel-border)] px-5 py-3">
              <button type="button" onClick={() => setEditing(null)} className="rounded-xl border border-[var(--lo-card-border)] px-4 py-2 text-xs font-bold text-[var(--lo-text)] transition hover:bg-[var(--lo-card-border)]">Cancel</button>
