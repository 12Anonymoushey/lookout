      ...
      {open && (
        <div className="absolute bottom-full right-0 mb-2 w-[min(20rem,calc(100vw-1.5rem))] overflow-hidden rounded-2xl border border-[var(--lo-card-border)] bg-[var(--lo-panel)]/95 shadow-2xl backdrop-blur">
          <header className="flex items-center justify-between gap-2 border-b border-[var(--lo-panel-border)] px-3 py-2">
            <p className="flex items-center gap-1.5 text-xs font-bold text-[#f0e6f6]">
              <Inbox size={13} style={{ color: 'var(--lo-pink)' }} /> Pokes for {plate}
            </p>
            <span className="rounded-full px-2 py-0.5 text-[10px] font-bold" style={{ background: 'var(--lo-cyan)', color: '#0e0a18' }}>
              {state.count}
            </span>
          </header>
          {state.list.length === 0 ? (
            <p className="px-3 py-5 text-center text-[11px] text-[#8a7098]">No pokes yet.</p>
          ) : (
            <ul className="max-h-72 divide-y divide-[var(--lo-panel-border)] overflow-y-auto">
              {state.list.map((poke) => (
                <li key={poke.id} className="flex items-start gap-2 px-3 py-2.5">
                  <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full" style={{ background: 'var(--lo-cyan)', color: '#0e0a18' }}>
                    <Hand size={12} />
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-[11px] font-bold text-[#f0e6f6]">{poke.fromName || 'A commuter'}</p>
                    <p className="text-[11px] text-[#d8c8e8]">{poke.message || 'Poke! 👋'}</p>
                    <p className="mt-0.5 flex items-center gap-1 text-[10px] text-[#8a7098]">
          ...
          <p className="border-t border-[var(--lo-panel-border)] px-3 py-1.5 text-[9px] uppercase tracking-widest text-[#8a7098]">
      ...
      <button
        ...
        className={`flex items-center gap-2 rounded-full border px-3.5 py-2 text-xs font-bold shadow-xl backdrop-blur transition ${
          bump
            ...
            : 'border-[var(--lo-card-border)] bg-[var(--lo-panel)]/90 text-[#e0d4ec] hover:border-[var(--lo-pink)]/40 hover:text-[var(--lo-pink)]'
        }`}
      >
